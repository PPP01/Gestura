...
| Was | Wo | Status |
|---|---|---|
| Upload-Paket (ZIP) | `web-ext-artifacts/gestura-2.7.0-chrome.zip` | ✅ dasselbe wie Chrome (~1 MB) |
...
Fertig: `web-ext-artifacts/gestura-2.7.0-chrome.zip`. Neu bauen (falls du auf `main`
...
Edge-Dashboard → **„New extension"** → `gestura-2.7.0-chrome.zip` hochladen. Edge
...
No data collection by default. Settings are stored locally via storage / storage.sync only; page content is processed on-device and never transmitted. The one exception is the optional, off-by-default gestura.eu integration feature: if the user turns it on in the options page, pages on gestura.eu (and one developer origin the user may configure) can ask the extension its version number and, for entries the user previously imported from that same origin, whether they are still installed, in which version, and whether they were changed locally. No account, no personal data, and nothing is sent while the user is not on such a page; the switch and consent are stored only locally and never synced. See PRIVACY.md, "gestura.eu integration".
...
- [ ] ZIP hochgeladen (`gestura-2.7.0-chrome.zip`)
