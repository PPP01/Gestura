...
A stored consent for an older version never authorizes anything — the switch shows as "needs re-confirmation" and all gated paths stay off until the user confirms the new consent text. For the two tiers the rule composes explicitly — tier 2 can never activate on its own consent object alone:

```text
integrationEnabled = euIntegration.enabled && euIntegration.consent.version === CURRENT_INTEGRATION_CONSENT
syncEnabled        = integrationEnabled && euSync.enabled && euSync.consent.version === CURRENT_SYNC_CONSENT
...
- **Limits and types (part of the contract):** the event `detail` string is at most **32 KiB of UTF-8**, checked **before** `JSON.parse` — a size-limited `ids` array must not smuggle in a huge irrelevant extra field. `requestId` is a string of at most 64 characters. `ids` is an array of at most 100 strings, each matching the exchange schema's ID rule (`js/exchange-schema.json`: its pattern, max 128 characters). Anything else — wrong types, over limit, malformed JSON — gets silence, like every other failure.
...
- **Re-import dedup, three cases:** a **qualified** import matches exclusively by `(indexOrigin, indexId)`. An **unqualified** import (file; hand-off or URL from a non-allowed origin — it has an `indexId` but no `indexOrigin`) matches by ID against other unqualified entries, but **never automatically overwrites a qualified entry**. When an ID is ambiguous (the same `indexId` exists qualified and/or from several origins), the import dialog shows the candidates or offers only "import as new entry" — it never guesses. Required test: the same ID present from production, dev origin and a file.
- **The update check is grouped by origin:** qualified entries are grouped by `indexOrigin`, and each allowed origin is asked its **own** `/api/v1/updates` — production entries go only to `https://gestura.eu`, dev-origin entries only to the configured dev origin. Neither ever appears in the other's request; unqualified entries appear in none.
...
**Update check (R2):** triggered when the options page opens, throttled to at most once per day — no background alarm, no traffic while settings are closed. `POST /updates` with the `{id, version}` list of entries with qualified index provenance (endpoint exactly as in the July design: answer only for entries with a newer version, including deprecation notices, no account binding). Results live in a local cache in `storage.local`, **kept per origin** — one shared `checkedAt` could not express "production answered, the dev index was down":

```json
{ "euUpdates": { "origins": [ { "origin": "…", "checkedAt": "…", "results": […] } ] } }
```

An array, not origin-keyed objects (no arbitrary strings as object keys). A successful, fully validated response replaces **only its own origin's slot** — so badges survive closing the options page instead of vanishing behind the throttle window. A failed or invalid response changes neither `results` nor `checkedAt` of that origin: **a network error does not start the 24-hour window**; the next options-page open simply tries that origin again. Disabling the integration deletes the whole cache; changing or removing the dev origin deletes that origin's slot. The badges render from the cache onto the affected entries; applying an update runs through the existing import dialog with diff. For `transformCode` changes the code diff is shown mandatorily with a fresh confirmation checkbox (July design, section 6, unchanged).
...
- Before every upload a **"What is transferred?" view** shows the complete JSON — Issue #1's transparency requirement. **This view does not exist yet and is R3 work:** today's settings export writes the file immediately (`#exportSettings` in [js/components/options-page.js](../../../js/components/options-page.js)) and the full-settings import is a bare `confirm()` whose validation checks little more than `enableGesture`. R3 therefore explicitly includes: a full settings **export preview**, a **download/import preview**, and a real **settings validator** — one validator, shared between the file import and the sync download (and reused by the file export/import path, which inherits the preview for free). Its ground rules, to be pinned exactly in the settings schema within `docs/gestura-eu-api.md`:
	...
	- a **settings format version of its own**: today's export carries only `_version`, which is the *extension* version (`#exportSettings`), not a format version. R3 introduces `"gesturaSettings": 1` as the field that drives validation and migration — `_version` stays purely informational; unknown format versions are refused with a clear message, and exports without the field go through an explicitly named **legacy validator**;
...
- Update check: network failure or non-200 → the affected origin's cache slot stays untouched and no throttle window starts — the next options-page open tries that origin again; never an error dialog for a background nicety.
...
- **Dedup:** the same `indexId` present from production, dev origin and a file — qualified imports match only their own origin's entry, the file import never overwrites a qualified entry automatically, ambiguity surfaces candidates or "import as new".
...
- **Update check:** request bodies contain exactly the `{id, version}` pairs of qualified entries, grouped per origin — file imports never appear, production and dev origins never mix; a validated success replaces only its own origin's `euUpdates` slot, a network error starts no throttle window for that origin, disabling the integration deletes the whole cache, changing the dev origin deletes its slot.
- **Settings validator:** unknown top-level keys are listed in the preview and never written; deeply nested `__proto__`/`constructor`/`prototype` keys are rejected before any object merge; `euIntegration`/`euSync`/secret in a crafted file are ignored.
- **Sync gating:** a valid tier-2 consent with tier 1 off or stale produces no sync communication at all.
