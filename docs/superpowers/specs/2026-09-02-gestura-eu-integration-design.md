...
**Update check (R2):** triggered when the options page opens, throttled to at most once per day — no background alarm, no traffic while settings are closed. `POST /updates` with the `{id, version}` list of entries with qualified index provenance (endpoint exactly as in the July design: answer only for entries with a newer version, including deprecation notices, no account binding). Results live in a local cache `euUpdates: { checkedAt, results }` in `storage.local`, replaced **only** after a successful, fully validated response — so badges survive closing the options page instead of vanishing behind the throttle window. A failed or invalid response changes neither `results` nor `checkedAt`: **a network error does not start the 24-hour window**; the next options-page open simply tries again. Disabling the integration deletes the cache. The badges render from the cache onto the affected entries; applying an update runs through the existing import dialog with diff. For `transformCode` changes the code diff is shown mandatorily with a fresh confirmation checkbox (July design, section 6, unchanged).
