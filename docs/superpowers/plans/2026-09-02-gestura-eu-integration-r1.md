...
- **The section's own copy is `en`/`de` only.** It was rewritten on 2026-09-02 to name gestura.eu instead of "websites" (below), by the owner's decision that translation waits until the text is final. Five keys need it:
  ...
  - `euIntegrationTitle`, `euIntegrationToggle` and `euIntegrationToggleDesc` changed wording. The other 37 locales still hold the **old** text ("Website integration", "Allow websites to talk to Gestura"). No test catches this — the keys exist, they are just stale.
