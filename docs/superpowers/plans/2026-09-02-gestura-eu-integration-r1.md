...
- ~~**The bridge's `getManifest()` and `statusAnswer()` calls sit outside a try/catch.**~~ Fixed 2026-09-02: both handlers are wrapped whole, because nothing in them is guaranteed not to throw — `getManifest()` fails on an invalidated context, storage can reject, and `reply()` touches `JSON.stringify` and the DOM. An unhandled rejection prints to the page's own console, which the page can observe, so it really did break the silence rule.
- ~~**`eu-local.js` never retries a failed first load.**~~ Fixed 2026-09-02: a failed read drops the memo so the next caller retries, and answers the defaults for that one call — every gated path must fail closed. Both failure shapes are covered: a rejected promise, and the synchronous throw `chrome.storage.local.get` produces on an invalidated context, which previously resolved to `{}` and marked the state loaded. `tests/eu-local.test.mjs` covers it, and it is **the first test file in the suite with a `chrome` stub** — the module calls `load()` at its own module scope, so the stub must exist before the import and one import is one instance. That closes part of the parked test-coverage item.
