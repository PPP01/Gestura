...
- **The section's own copy is `en`/`de` only.** It was rewritten on 2026-09-02 to name gestura.eu instead of "websites" (below), by the owner's decision that translation waits until the text is final. Five keys need it:
  - `euIntegrationIntro` and `euIntegrationIntroLink` are new and exist only in `en`/`de`; they are listed in `PENDING_TRANSLATION` in `tests/site-menu-locales.test.mjs`, and a test there fails once they are translated everywhere but still listed. Every other locale falls back to `en` at runtime in the meantime.
  - `euIntegrationTitle`, `euIntegrationToggle`, `euIntegrationToggleDesc` and `euIntegrationConsentPoint1` changed wording. The other 37 locales still hold the **old** text ("Website integration", "Allow websites to talk to Gestura", and a consent point that still says *websites* may hand entries over — which is no longer true). No test catches this: the keys exist, they are just stale. The consent point is the one that matters most, since it is what the user agrees to.
