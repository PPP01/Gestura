...
4. ~~**`AGENTS.md` has drifted from `CLAUDE.md`.**~~ **Done 2026-09-03:** reduced to a pointer at `CLAUDE.md`, which now says so itself. A diff confirmed the copy held nothing of its own beyond its header — what it was missing was the `PENDING_TRANSLATION` paragraph (the false finding) and the entire `git merge` section, which the Firefox merge that same day needed.
